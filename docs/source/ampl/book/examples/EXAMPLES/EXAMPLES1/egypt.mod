...
set commod := c_final union c_inter union c_raw;   # All commodities
...
param exch;        # Exchange rate
param util_pct;    # Utilization percent for initial capacity
...
set p_prohib {plant} within proc;
                   # At each plant, list of processes that are
                   # arbitrarily ruled out
set p_pos {pl in plant} := p_cap[pl] diff p_prohib[pl];
...
set p_prohib[ASWAN] := CAN_335 ;
set p_prohib[HELWAN] := CAN_310 ;
set p_prohib[ASSIOUT] := ;
set p_prohib[KAFR_EL_ZT] := ;
set p_prohib[ABU_ZAABAL] := ;
