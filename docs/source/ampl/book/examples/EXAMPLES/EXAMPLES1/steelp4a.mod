...
param backcost {PROD} >= 0;     # carrying cost per ton of backorders
param revenue {PROD,1..T} >= 0; # revenue per ton sold

var Make {PROD,1..T} >= 0;                  # tons produced
var Inv {PROD,0..T} >= 0;                   # tons inventoried
var Back {PROD,0..T} >= 0;                  # tons backordered
...
maximize total_profit: 
   sum {p in PROD, t in 1..T} (revenue[p,t]*Sell[p,t] -
      prodcost[p]*Make[p,t] - invcost[p]*Inv[p,t] - backcost[p]*Back[p,t]) -
...
subject to initial {p in PROD}:  Inv[p,0] - Back[p,0] = inv0[p];
