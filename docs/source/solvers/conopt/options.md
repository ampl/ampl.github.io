...
frq_log_simple    frequency for log-lines for non-SLP/SQP iterations
frq_log_slpsqp    frequency for log-lines for SLP or SQP iterations
flg_negcurve      test for negative curvature near optimum (0/1*)
flg_nodefc        turn definitional constraints off (0*/1)
flg_nopen         allow model without penalty constraints (0/1*)
frq_rescale       rescaling frequency (int, default 5)
flg_slpmode       enable slp mode (0/1*)
flg_sqpmode       enable sqp mode (0/1*)
flg_square        square system (0*/1)
flg_tracecns      trace cns solution (0*/1)
...
maxtime           time limit (default no limit)
