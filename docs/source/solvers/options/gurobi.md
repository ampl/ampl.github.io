...
pl_bigm          When some variables appear in piecewise-linear terms in the
...
pool_distmip     Number of machines in the server pool (if specified by
                 pool_servers) to use for solving each MIP instance.

pool_mip         Number of independent MIP jobs (default 0) to generate and
...
pool_servers     Comma-separated list of server names or IP addresses
                 of machines in the server pool (default "" = none).

pool_tunejobs    Number of parallel tuning jobs (default 0) to run on the
